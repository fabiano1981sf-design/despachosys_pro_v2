...
export default function ContasReceber() {
  const [formData, setFormData] = useState({
    ...
    clienteId: "",
    planoContasId: "",
    status: "aberto" as "aberto" | "recebido" | "vencido" | "cancelado",
    observacao: "",
  });
  const [editingId, setEditingId] = useState<number | null>(null);

  const utils = trpc.useUtils();
  const { data, isLoading } = trpc.contasAReceber.list.useQuery();
  const { data: clientes } = trpc.clientes.list.useQuery();
  const { data: planoContas } = trpc.planoContas.list.useQuery();

  const createMutation = trpc.contasAReceber.create.useMutation({
    onSuccess: () => {
      toast.success("Conta criada!");
      utils.contasAReceber.list.invalidate();
  ...
  const updateMutation = trpc.contasAReceber.update.useMutation({
    onSuccess: () => {
  ...
  const deleteMutation = trpc.contasAReceber.delete.useMutation({
    onSuccess: () => {
  ...
  const handleSubmit = (e: React.FormEvent, onClose: () => void) => {
    ...
    const data = {
      ...
      dataRecebimento: formData.dataRecebimento ? new Date(formData.dataRecebimento) : undefined,
  ...
  return (
    <CrudTable
      ...
      columns={[
        ...
        { header: "Recebimento", accessor: (row) => formatDate(row.dataRecebimento) },
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Select value={formData.planoContasId} onValueChange={(v) => setFormData({ ...formData, planoContasId: v })}>
                ...
                <SelectContent>
                  <SelectItem value="none">Nenhum</SelectItem>
                  {planoContas?.filter(c => c.tipo === "receita").map((conta) => (
